...
Zero ranges are skipped rather than written, so a raw target has to be sized by
the caller:
```go
f, _ := os.Create("a.raw")
defer f.Close()
_ = f.Truncate(img.Size())
_ = convert.Convert(f, img, convert.Options{})
```

A `vhdx.Writer` sizes its own file:
```go
