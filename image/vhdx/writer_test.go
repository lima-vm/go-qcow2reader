...
func TestWriterInvalidOptions(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			// Validate must reject everything that NewWriter rejects, so that a
			// caller can check the size and the options before creating the
			// target file.
			opts := vhdx.WriterOptions{BlockSize: tc.blockSize}
			if err := opts.Validate(tc.size); err == nil {
				t.Errorf("expected Validate error for size %d, block size %d", tc.size, tc.blockSize)
			}
			if _, err := vhdx.NewWriter(f, tc.size, vhdx.WriterOptions{BlockSize: tc.blockSize}); err == nil {
				t.Errorf("expected NewWriter error for size %d, block size %d", tc.size, tc.blockSize)
			}
			// A failed NewWriter must not write anything to the target.
			st, err := f.Stat()
			if err != nil {
				t.Fatal(err)
			}
			if st.Size() != 0 {
				t.Errorf("expected empty target, got %d bytes", st.Size())
			}
		})
	}
}
