...
// newRandomGUID returns a random (version 4) GUID. [rand.Read] never fails,
// crashing the program instead, so there is no error to return.
...
// Validate validates the virtual disk size and the options, and sets default
// option values. [NewWriter] calls it, but a caller can call it earlier to
// check the size and the options before creating the target file.
func (o *WriterOptions) Validate(size int64) error {
	if size <= 0 {
		return fmt.Errorf("virtual disk size %d must be positive", size)
	}
	if size > MaxVirtualDiskSize {
		return fmt.Errorf("virtual disk size %d exceeds the maximum %d", size, MaxVirtualDiskSize)
	}
	if size%LogicalSectorSize != 0 {
		return fmt.Errorf("virtual disk size %d is not a multiple of the sector size %d", size, LogicalSectorSize)
	}
	if o.BlockSize == 0 {
...
// needed. Ranges that are never written read as zeros, and a write of all
// zeros to a block that is not allocated yet is skipped, leaving the block
// unallocated, so unallocated and zero extents of a source image become
// unallocated blocks in the VHDX image. This makes Writer a suitable target
// for [github.com/lima-vm/go-qcow2reader/convert.Convert].
//
// A block is never deallocated: writing zeros over a block that is already
// allocated stores the zeros in the file.
//
...
func NewWriter(w io.WriterAt, size int64, opts WriterOptions) (*Writer, error) {
	...
	wr.fileWriteGUID = newRandomGUID()
	wr.dataWriteGUID = newRandomGUID()
	wr.virtualDiskID = newRandomGUID()
...
func (wr *Writer) Close() error {
	...
	// must read as zero, so writing a single zero byte extends the file without
	// writing the entire range.
...
func (wr *Writer) metadata() []byte {
	...
	b := make([]byte, int(metadataItemsOffset)+itemsLength)
